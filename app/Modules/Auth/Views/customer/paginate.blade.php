...
<div class="d-flex align-items-center">
    <span class="text-muted">
        {{ trans('Rule::permission.list.paginate.display') }}
        {{ trans('Rule::permission.list.paginate.from') }}
        {{ $paginator->firstItem() }}
        {{ trans('Rule::permission.list.paginate.to') }}
        @if($paginator->lastPage())
        {{ $paginator->lastItem() }}
        @else
        {{ $paginator->firstItem() + $paginator->perPage() - 1 }}
        @endif
        {{ trans('Rule::permission.list.paginate.of') }}
        {{ $paginator->total() }}
    </span>
</div>
