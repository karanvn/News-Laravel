...
<html lang="en">
	...
	<head>
        <base href="{{ asset('') }}">
		<meta charset="utf-8" />
		<title>{{ trans('Auth::auth.login.title') }}</title>
				...
				<div class="d-flex flex-center bgi-size-cover bgi-no-repeat flex-row-fluid" style="background-image: url(admin/assets/media/bg/bg-1.jpg);">
					<div class="login-form text-center text-white p-7 position-relative overflow-hidden bg-light-white rounded opacity-100 ml-10 mr-10"> 
						...
						<div class="login-signin">
                            <div class="mb-10">
                                <h3 class="font-weight-normal text-primary">{{ __('Auth::auth.login.header') }}</h3>
                                @if (Session::has('message'))
                                    <p class="opacity-100 error" style="color:red">{{ Session::get('message') }}</p>
                                @endif
							</div>
							<form class="form" id="frmLogin" onsubmit="kt_login_signin_submit.disabled=true; return true;" method="POST" action="{{ route('login') }}">
                                @csrf
								...
								<div class="form-group" style="margin-top: 15px">
									<input id="password" class="form-control form-control-solid h-auto text-primary bg-white-o-5s rounded border-0 py-4 px-8" type="password" placeholder="{{ trans('Auth::auth.login.password') }}" name="password" />
								</div>
								{{--<div class="form-group d-flex flex-wrap justify-content-between align-items-center px-8 opacity-60">
									<div class="checkbox-inline">
										<label class="checkbox checkbox-outline checkbox-white text-white m-0">
										<input type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }} />
										<span></span>{{ trans('Auth::auth.login.remember') }}</label>
                                    </div>
                                    @if (Route::has('UserForget'))
                                        <a id="kt_login_forgot" class="text-white font-weight-bold" href="{{ route('UserForget') }}">
                                            {{ trans('Auth::auth.login.forget_password') }}
                                        </a>
                                    @endif
                                </div>--}}
                                <a class="text-primary" href="{{ route('Reset') }}">{{ trans('Auth::auth.reset.forget_password') }} ?</a>
								<div class="form-group text-center mt-5">
                                    <button type="submit" id="kt_login_signin_submit" name="kt_login_signin_submit" class="btn btn-primary opacity-80 px-15 py-3">{{ trans('Auth::auth.login.submit') }}</button>
								</div>
							</form>
						</div>
