...
<html lang="en">
	...
	<head>
        <base href="{{ asset('') }}">
		<meta charset="utf-8" />
		<title>{{ trans('Auth::auth.password.title') }}</title>
		...
		<div class="d-flex flex-column flex-root">
			...
			<div class="login login-5 login-signin-on d-flex flex-row-fluid" id="kt_login">
				<div class="d-flex flex-center bgi-size-cover bgi-no-repeat flex-row-fluid" style="background-image: url(admin/assets/media/bg/bg-1.jpg);">
					<div class="login-form text-center text-white p-7 position-relative overflow-hidden bg-light-white rounded ml-10 mr-10">
						<!--begin::Login Header-->
						...
						<div class="login-signin">
                            <div class="mb-10">
                                <h3 class="font-weight-normal text-primary">{{ __('Auth::auth.password.header') }}</h3>
							...
							<form class="form" id="frmResetPassword" onsubmit="kt_login_signin_submit.disabled=true; return true;" method="POST" action="{{ route('ResetPassword') }}">
								...
								<div class="form-group">
									<input id="email" class="form-control form-control-solid h-auto text-primary bg-white-o-5s rounded border-0 py-4 px-8" type="email" placeholder="{{ trans('Auth::auth.password.email') }}" name="email"  autocomplete="off" />
                                </div>
                                <div class="form-group">
									<input id="password" class="form-control form-control-solid h-auto text-primary bg-white-o-5s rounded border-0 py-4 px-8" type="password" placeholder="{{ trans('Auth::auth.password.password') }}" name="password" autocomplete="off" />
                                </div>
                                <div class="form-group">
									<input id="re_password" class="form-control form-control-solid h-auto text-primary bg-white-o-5s rounded border-0 py-4 px-8" type="password" placeholder="{{ trans('Auth::auth.password.re_password') }}" name="re_password" autocomplete="off" />
                                </div>
                                <a class="text-primary" href="{{ route('login') }}">{{ trans('Auth::auth.login.msg_login') }}</a>
								<div class="form-group text-center mt-5">
									<button type="submit" id="kt_login_signin_submit" name="kt_login_signin_submit" class="btn btn-primary opacity-80 px-15 py-3">{{ trans('Auth::auth.password.submit') }}</button>
								</div>
							</form>
						</div>
