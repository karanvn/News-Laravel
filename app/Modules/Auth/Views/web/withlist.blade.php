@extends('home.main')
@section('title')
{{ trans('Auth::customer.withlist.title') }}
@endsection
@section('content')
{{--  start-Breadcrumb  --}}
<div class="banner-wrapper">
    <div class="banner-wrapper-inner">
        <h1 class="page-title  text-uppercase">{{ trans('Auth::customer.withlist.title') }}</h1>
    </div>
</div>
{{--  end-Breadcrumb  --}}
{{-- list --}}
<div class="container">
@if(count($datas)>0)
<table class="shop_table cart wishlist_table" id="listWithList">
    <thead>
    <tr>
...
@php continue; @endphp
@endif
 <tr id="yith-wcwl-row-29" id="row_{{$product->product_id}}">
...
<button class="btnallNew" id="seemore">{{ trans('Auth::customer.withlist.seemore') }}</button>
</div>
@endif
<script>
    $('#seemore').on('click', function(){
        $(".animationloadpage").show();
	    $.ajax("/user/withlistajax?page="+(Number($('#currentPage').val()) + 1), {
		method: "GET",
		dataType: "json",
		contentType: !1,
		cache: !1,
...
console.log(a);
            $('#listWithList').html( $('#listWithList').html() + '' + a.html);
            if(a.status=='0'){
               $('#seemore').hide(); 
            }
