...
<div class="d-flex align-items-center">
    <span class="text-muted">
        {{ trans('Partner::partner.list.paginate.display') }}
        {{ trans('Partner::partner.list.paginate.from') }}
        {{ $paginator->firstItem() }}
        {{ trans('Partner::partner.list.paginate.to') }}
        @if($paginator->lastPage())
        {{ $paginator->lastItem() }}
        @else
        {{ $paginator->firstItem() + $paginator->perPage() - 1 }}
        @endif
        {{ trans('Partner::partner.list.paginate.of') }}
        {{ $paginator->total() }}
    </span>
</div>
