...
<div class="container mt-5" id="list_collections">
	
@if(count($collections)>0)

@foreach ($collections as $collection)

<div class="lynessa-heading style-01 mt-5" data-aos="fade-up">
	<div class="heading-inner">
		<a href="/{{@$collection->slug}}.html">
			<h2 class="title">{{@$collection->name}}<span></span></h2>
		</a>
		{{-- <div class="subtitle"><a href="/{{@$collection->slug}}.html">Xem thêm</a></div> --}}
	</div>
</div>
<div class="row">
	@php $stt = 0; @endphp
	@php $productsColection = $collection->products; @endphp
	@if(count($productsColection)>0)
@foreach($productsColection as $product)
@if($stt >= 4)
	@php
		break;
	@endphp
...
<script>
	...
	$('#seeMoreCollection').on('click', function(){
			...
			$.ajax("/loadCollectionsPage/?page=" + page, {
			method: "GET",
			dataType: "json",
			contentType: !1,
			cache: !1,
			...
			page++;
			$('#list_collections').append(a.html);
			if(a.success == '2'){
				$(this).hide();
				$('#seeMoreCollection').remove();
			}
