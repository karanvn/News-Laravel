...
<div class="d-flex align-items-center">
    <span class="text-muted">
        {{ trans('Banner::banner.list.paginate.display') }}
        {{ trans('Banner::banner.list.paginate.from') }}
        {{ $paginator->firstItem() }}
        {{ trans('Banner::banner.list.paginate.to') }}
        @if($paginator->lastPage())
        {{ $paginator->lastItem() }}
        @else
        {{ $paginator->firstItem() + $paginator->perPage() - 1 }}
        @endif
        {{ trans('Banner::banner.list.paginate.of') }}
        {{ $paginator->total() }}
    </span>
</div>
