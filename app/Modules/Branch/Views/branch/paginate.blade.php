...
<div class="d-flex align-items-center">
    <span class="text-muted">
        {{ trans('Branch::branch.list.paginate.display') }}
        {{ trans('Branch::branch.list.paginate.from') }}
        {{ $paginator->firstItem() }}
        {{ trans('Branch::branch.list.paginate.to') }}
        @if($paginator->lastPage())
        {{ $paginator->lastItem() }}
        @else
        {{ $paginator->firstItem() + $paginator->perPage() - 1 }}
        @endif
        {{ trans('Branch::branch.list.paginate.of') }}
        {{ $paginator->total() }}
    </span>
</div>
