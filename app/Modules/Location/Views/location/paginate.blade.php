...
<div class="d-flex align-items-center">
    <span class="text-muted">
        {{ trans('Location::state.list.paginate.display') }}
        {{ trans('Location::state.list.paginate.from') }}
        {{ $paginator->firstItem() }}
        {{ trans('Location::state.list.paginate.to') }}
        @if($paginator->lastPage())
        {{ $paginator->lastItem() }}
        @else
        {{ $paginator->firstItem() + $paginator->perPage() - 1 }}
        @endif
        {{ trans('Location::state.list.paginate.of') }}
        {{ $paginator->total() }}
    </span>
</div>
