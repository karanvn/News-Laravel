...
<div class="d-flex align-items-center">
    <span class="text-muted">
        {{ trans('Policy::policy.list.paginate.display') }}
        {{ trans('Policy::policy.list.paginate.from') }}
        {{ $paginator->firstItem() }}
        {{ trans('Policy::policy.list.paginate.to') }}
        @if($paginator->lastPage())
        {{ $paginator->lastItem() }}
        @else
        {{ $paginator->firstItem() + $paginator->perPage() - 1 }}
        @endif
        {{ trans('Policy::policy.list.paginate.of') }}
        {{ $paginator->total() }}
    </span>
</div>
